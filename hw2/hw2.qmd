...
```{r}
admissions_tble <- icustays_tble %>% 
  semi_join(admissions_tble, by = c("subject_id", "hadm_id"))
```
...
labevents_tble <- str_c(mimic_path, "/hosp/labevents.csv.gz") %>% 
  read_csv(col_select = c(subject_id, itemid, charttime, valuenum)) %>% 
...
```{r, message = FALSE}
chartevents_tble <- str_c(mimic_path, "/icu/chartevents.csv.gz") %>% 
  read_csv(col_select = c(subject_id, itemid, charttime, valuenum)) %>% 
