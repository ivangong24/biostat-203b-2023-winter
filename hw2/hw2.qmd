...
Most patients will be discharged within 10 days, however, there are a few of them stayed over 100 days.
...
mimic_icu_cohort %>% 
  select(thirty_day_mort, ethnicity, language, 
         insurance, marital_status, gender, anchor_age)
    ...
    function(data1, data2){
      mimic_icu_cohort %>% 
        set_value_labels(thirty_day_mort = c("Died within 30 days" = 1, 
                         ...
                         gender = c("Male" = "M", "Female" = "F")
                         ) %>% 
        modify_if(is.labelled, to_factor) %>% 
        select(thirty_day_mort, data1) %>% 
        # mutate_at(vars(data1), str_to_title) %>%
        ggplot(aes(x = thirty_day_mort, fill = .[, 2])) + 
